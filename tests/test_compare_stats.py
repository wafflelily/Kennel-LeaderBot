...
class TestGauntleCompare:
    ...
    def test_lines(self, cog, rows):
        lines = cog.compare_stats(rows, P1)
    ...
    def test_old_format_category_row_can_hold_a_best(self, cog, rows):
        # P2's Mines (old-format float, 80s) beats P1's 90s.
        lines = cog.compare_stats(rows, P2)
        assert "**Mines**" in lines[3]
        assert "(1 of 3)" in lines[3]

    def test_no_bests_held(self, cog):
        rows = [
    ...
    def test_same_day_reposts_keep_fastest_run(self, cog):
        rows = [
    ...
    def test_unknown_player_returns_none(self, cog, rows):
        assert cog.compare_stats(rows, 999) is None

    def test_empty_rows_return_none(self, cog):
        assert cog.compare_stats([], P1) is None
...
class TestFoodGuessrCompare:
    ...
    def test_perfects_line_omitted_when_channel_has_none(self, cog):
        rows = [
    ...
    def test_same_day_reposts_keep_best_score(self, cog):
        rows = [
            row(P1, date(2026, 6, 1), {"score": 9000}),
...
class TestCatfishingCompare:
    ...
    def test_solo_play_gives_zero_unique_answers(self, cog):
        rows = [row(P1, date(2026, 8, 1), {"puzzle": 700, "score": 5.0, "correct": [0, 1, 2, 3, 4]})]
        lines = cog.compare_stats(rows, P1)
        assert "**0** across 0 shared puzzles" in lines[1]

    def test_reposts_of_a_puzzle_union_their_answers(self, cog):
        rows = [
