...
A Discord bot that reads daily-game results ("dles") posted in your channels and turns them into monthly leaderboards. It currently tracks **Catfishing**, **FoodGuessr**, **Gauntle**, and **Krillion**, and can also parse an introductions channel to build a **"who invited whom" invite tree**.
...
| `/gauntle [month]` | Leaderboard for Gauntle runs posted in this channel. Shows the top 3 fastest overall runs and a per-category table with, for each of the 11 puzzle categories, the best effective time, the actual solve time with its bonus/penalty (e.g. `0:50.58 (−10s)`), and who set it. When you post a run that's a new personal best — overall time or in any category — the bot replies with a small congratulations. |
| `/krillion [month]` | Leaderboard for [Krillion](https://krillion.io) scores posted in this channel. Ranks players by total points (best score per day) with days played and average, plus each player's catch counts — 🌟 shrimp, 🏮 lanternfish, 🦑 squid. For days at least two people shared a grid, it also lists the questions the group did best and worst at on average, naming a prompt's 🌟 shrimp answer only when someone in the channel actually caught it (answers from the bot's daily archive, so only days since deployment can be named). Pack results are ignored; only the daily game counts. |
| `/mystats [game]` | How *you* stack up against everyone tallied in this channel (the game sites already show plain personal stats, so this is all comparative): days you had the top result, Catfishing answers nobody else got — including your 5 hardest and 5 easiest unique solves ranked by global solve rate — your average vs the channel's, your Gauntle personal best run and per-category bests (👑 marks the ones that are the channel record), your share of the channel's FoodGuessr perfects, and a tally of your Krillion catches by answer quality plus the archived questions you caught a 🌟 shrimp on, each naming the star answer (or answers, for the rare prompt with more than one). Covers everything cached for this channel, across all games unless you name one. |
...
| `/import <game> <channel>` | One-off: scan another channel's full history and merge its results for a game into *this* channel's data — for when results used to be posted somewhere else. Re-running is safe (keyed by message, nothing duplicates), but the source channel isn't tracked afterwards, and `/rebuild` wipes imported rows along with the rest (re-run `/import` after). |
...
- **Krillion**: the standard share text — a `Krillion #47 🦐` header, the score on its own line, and optionally the 7-emoji result grid (⬛ 0, 🫧 10, 🐟 30, 🦑 60, 🏮 85, 🌟 100). A bare `#47` + score also works. Pack results (an emoji between "Krillion" and the number, e.g. `Krillion 🎬 #1 🦐`) are deliberately not counted.
...
- `cogs/catfishing.py`, `cogs/foodguessr.py`, `cogs/gauntle.py`, `cogs/krillion.py`, `cogs/introductions.py` — the five "leaderboard" cogs, all built on a shared engine.
...
- **Krillion** dates results the same way Catfishing does — shares carry a day number, not a date, so the shared puzzle-number anchor attributes each result to its real day. The 7-emoji grid is authoritative for the score when present; the bare `#day` + score form is validated strictly (0–700, multiple of 5, whole message) so ordinary chat can't match, and pack shares are rejected because the pack emoji breaks the `Krillion #N` header pattern. The cog also runs an **archiver**: krillion.io only serves the *current* day's data — past days are refused to everyone, so it's date-gated, not login-gated — so a daily task (plus a catch-up on startup) stores each day while it's live in the `puzzle_info` table. It reads the day number from `/api/today` and the full answer sheet from the public `/api/reveal?date=<today>`, keeping the prompt texts and each prompt's shrimp (100-point) answers. A prompt usually has a single star answer, so a 🌟 in someone's grid pins down the exact answer they gave; where a prompt has several, all are listed. This powers the `/mystats` shrimp-catch list and the leaderboard's best/worst-question fields. Days from before the bot started archiving simply don't get named.
...
- **Gauntle** stores each category's raw solve time and its bonus/penalty adjustment (including the skip penalty) separately; rankings use the *effective* time (raw + adjustment), and the per-category table shows both — the effective time and the actual solve with its adjustment. The share text states the run's date without a year ("I ran the June 18th Gauntlet…"), so the bot infers it: it picks whichever year makes that date fall closest to the day the message was posted — which gets December/January boundary posts right. It also hooks live capture: a freshly posted run is compared against the poster's own history, and a new overall best and/or new category bests get a small reply (first-ever runs and first-time categories just set the baseline silently; history scans never replay old bests).
...
| `puzzle_info` | Per-puzzle info fetched from a game's website, e.g. Krillion's daily prompt texts and shrimp answers (archived on the day, since the site doesn't serve past days). Not rebuildable — a missed day is gone. |
